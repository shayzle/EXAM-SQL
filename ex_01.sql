-- 1. Comment afficher toutes les informations d’une table ? (syntaxe complète)
    SELECT * FROM NOM_TABLE; -- an exemple : SELECT * FROM users;

-- 2. Quelle commande permet de sélectionner la base de données à utiliser ?


-- 3. Expliquer quelle est la différence entre SELECT * et SELECT colonne ?
    SELECT * -- = c'est une facon où nous voulons récupérer ou montrer toutes les informations d'une table.
...
-- 7. Quelle est la différence entre AND et OR ?
    -- la différence entre AND et OR :
        -- AND = c'est quand nous voulons chercher une valeur en mettant tous les deux informations qui sont necessaires pour trouver la vraie valeur.
...
-- 9. Quelle est la différence entre DROP et DELETE ?
    -- la différence entre DROP et DELETE :
        -- DROP = c'est quand nous voulons supprimer une valeur, mais pas dans le sens/context définitivement dans une table de database.
        -- DELETE = c'est quand nous voulons supprimer une valeur définitivement dans une table de database.
